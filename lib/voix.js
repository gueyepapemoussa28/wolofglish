...
// Au-delà, mieux vaut une autre voix qu'une belle voix qui n'arrive jamais :
// la synthèse de Gemini est passée de 5 à 49 secondes sur ce projet, frôlant
// la coupure des fonctions Vercel à 60 s — l'appel échouait alors en silence.
  ...
  for (const modele of modeles) {
    ...
    try {
      ...
      dernierDetail =
        err && err.name === "AbortError"
          ? modele + " a dépassé " + DELAI_MAX_MS / 1000 + " s."
          : "Appel à " + modele + " impossible : " + ((err && err.message) || "erreur inconnue");
      continue;
    } finally {
    ...
    if (reponse.ok) {
      ...
      return {
        ok: true,
        modele,
        audio: emballerEnWav(Buffer.from(partie.data, "base64"), taux),
        type: "audio/wav",
      };
    }
