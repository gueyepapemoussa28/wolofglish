...
async function synthetiser(texte) {
  ...
  for (const porte of PORTES) {
    ...
    try {
      ...
      if (!reponse.ok) {
        tentatives.push(
          porte.url.split("/").slice(3).join("/") + " → " + reponse.status + " " +
          (await reponse.text()).slice(0, 150)
        );
        continue;
      }
      ...
      if (type.indexOf("application/json") !== -1) {
        ...
        tentatives.push(
          porte.url.split("/").slice(3).join("/") + " → JSON sans audio : " +
          JSON.stringify(donnees).slice(0, 150)
        );
        continue;
      }
      ...
      if (audio.length < 1000) {
        tentatives.push(porte.url.split("/").slice(3).join("/") + " → audio vide");
        continue;
      }
      return { ok: true, audio, type: type || "audio/wav", porte: porte.url };
    } catch (err) {
      tentatives.push(
        porte.url.split("/").slice(3).join("/") + " → " +
  ...
  return { ok: false, details: tentatives.join("  ||  ") || "Aucune porte essayée." };
}
